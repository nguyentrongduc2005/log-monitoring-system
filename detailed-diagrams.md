...
sequenceDiagram
    ...
    participant Redis as Redis Cache (D4)
    participant Kafka as Hàng đợi Kafka (D1)
    participant Alerting as Alerting Module (P3)
    ...
    par Stream Logs
        ...
        Kafka->>Gateway: Consume live log event (LiveLogKafkaConsumer)
        Gateway->>Op: Đẩy live log xuống Dashboard (WebSocket)
    and Stream Alerts & Anomalies
        Note over Alerting: Phát hiện Cảnh báo hoặc Bất thường mới
        Alerting->>Gateway: Gọi publishAlertNotification() / publishAnomalyReportNotification() (RealtimeFacade)
        Gateway->>Op: Gửi cảnh báo đỏ / bất thường (WebSocket)
    end
    deactivate Gateway
````
...
database "Redis Cache (D4)" as Redis
queue "Hàng đợi Kafka (D1)" as Kafka
participant "Alerting Module (P3)" as Alerting
...
par Stream Logs
    Kafka -> Gateway : Consume event từ topic "logs.live" (LiveLogKafkaConsumer)
    Gateway -> Op : Đẩy live log xuống Dashboard (WebSocket)
else Stream Alerts & Anomalies
    Note over Alerting : Phát hiện Cảnh báo hoặc Bất thường mới
    Alerting -> Gateway : Gọi publishAlertNotification() / publishAnomalyReportNotification() (RealtimeFacade)
    Gateway -> Op : Gửi cảnh báo đỏ / bất thường (WebSocket)
end
